...
if (menuItems.length > 0) {
    document.addEventListener('click', function (e) {
        ...
        if (subItemActive) {
            if (!target.closest('._item._change') && !target.closest(".-day-")) {
...
function calendarViewChange() {
    const prev = document.querySelector('[data-action="prev"]');
    prev.classList.add("air-datepicker-nav--action_prev")
    const next = document.querySelector('[data-action="next"]');
    next.classList.add("air-datepicker-nav--action_next")
}
if (document.querySelector('._calendar')) {
    let dp = new AirDatepicker('._calendar', {
        // minDate: `${date.getFullYear()}, 0${date.getMonth()+1}, ${date.getDate()+2}`,
        // dateFormat: 'dd MMMM yyyy',
        // visible: true,
        prevHtml: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M9.00002 3.828L14.364 9.192L15.778 7.778L8.00002 -9.27517e-08L0.222015 7.778L1.63602 9.192L7.00002 3.828L7.00002 16L9.00002 16L9.00002 3.828Z" fill="#0D0D0D"/>
    </svg>`,
        nextHtml: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M8.99998 12.172L14.364 6.808L15.778 8.222L7.99998 16L0.221985 8.222L1.63598 6.808L6.99998 12.172L6.99998 -3.83699e-07L8.99998 -2.96276e-07L8.99998 12.172Z" fill="#0D0D0D"/>
    </svg>`,
        navTitles: {
            days: 'MMMM yyyy',
        },
    ...
    document.addEventListener("click", function (e) {
        const target = e.target
        if (target.closest(".form__icon_calendar")) {
            if (document.querySelector('.air-datepicker-global-container').children.length > 0) {
                dp.hide();
            } else {
                dp.show();
            }
...
document.addEventListener("click", function (e) {
    ...
    if (target.closest(".form__eye")) {
        const parentEye = target.closest('.form__input_password');
        const parentEyeInput = parentEye.querySelector('.input');
        if (!parentEye.classList.contains("_show")) {
            parentEye.classList.add('_show')
            parentEyeInput.type = "text";
        } else {
            parentEye.classList.remove('_show')
            parentEyeInput.type = "password";
        }
        // if (parentEye.value != "") {
        //     parentEyeInput.focus();
        // }
    }
    // *search
    if (target.closest(".search__icon") && target.closest(".search_appear")) {
        const searchForm = target.closest(".search_appear");
        if (!searchForm.classList.contains('_show')) {
            searchForm.classList.add('_show')
            setTimeout(() => {
            ...
            setTimeout(() => {
                searchForm.classList.remove('_show')
            }, 300);
        }
        e.preventDefault();
    } else {
        if (!target.closest(".search_appear") && document.querySelector('.search_appear._show')) {
            document.querySelector('.search_appear._show').classList.remove('_active');
            setTimeout(() => {
                document.querySelector('.search_appear._show').classList.remove('_show')
            }, 300);
        }
    }
    // *Spoller close
    if (document.querySelector('._active[data-spoller]')) {
        // if (!target.closest("._active[data-spoller]") && !target.closest("[style=\"box-sizing: border-box;\"]")) {
        if (!target.closest("[data-spollers]")) {
            const slideTitle = document.querySelector('._active[data-spoller]');
            if (slideTitle.nextElementSibling && !slideTitle.nextElementSibling.classList.contains('_slide')) {
                slideTitle.classList.remove('_active')
                _slideUp(slideTitle.nextElementSibling, 500);
            }
        }
    }
    // *fileImage
    if (target.closest(".file")) {
        if (target.closest(".file__image") || target.closest(".file__remove")) {
            // target.closest(".file__image").remove();
            const fileParent = target.closest(".file");
            const fileImage = fileParent.querySelector('.file__image');
            const fileContent = fileParent.querySelector('.file__content');
            fileImage.remove();
            fileContent.remove();
            // target.closest(".file__image").hidden = true;
            e.preventDefault();
        } else if (target.closest(".file__content") && !target.closest(".file__remove")) {
            e.preventDefault();
        }
    }
});
// *tittle with background correct view
function titleCorrect() {
    const titles = document.querySelectorAll('._title__content');
    if (titles.length > 0) {
        for (let index = 0; index < titles.length; index++) {
            const title = titles[index];
            const titleText = title.querySelector('._title__text');
            title.style.marginLeft = (offset(title).left - offset(titleText).left) + "px";
            // title.style.paddingLeft = (offset(title).left - offset(titleText).left) + "px";
        }
    }
}
window.addEventListener("load", function () {
    ...
    if (document.querySelector('[data-action="prev"]')) {
        calendarViewChange();
    }
});
...
// const form = document.querySelector('formmain-lk__form');
const form = document.forms.main;
const checkboxControlParents = document.querySelectorAll('.form__checkboxes[data-limit]');
if (checkboxControlParents.length > 0) {
    for (let index = 0; index < checkboxControlParents.length; index++) {
        const checkboxControlParent = checkboxControlParents[index];
        let total = 0;
        const checkboxInputs = checkboxControlParent.querySelectorAll('.checkbox__input');
        if (checkboxInputs.length > Number(checkboxControlParent.dataset.limit)) {
            for (let index = 0; index < checkboxInputs.length; index++) {
                const checkboxInput = checkboxInputs[index];
                ...
                checkboxInput.addEventListener("change", function (e) {
                    ...
                    if (total > Number(checkboxControlParent.dataset.limit)) {
                        checkboxInput.checked = false;
                        total--;
                    }
                    // console.log(checkboxControlParent.querySelectorAll('.checkbox__input[checked]').length);
                });
            }
        }
    }
...
if (document.querySelector('.dates-home__slider-body')) {
    new Swiper('.dates-home__slider-body', {
        observer: true,
        observeParents: true,
        speed: 800,
        watchOverflow: true,
        // Arrows
        navigation: {
            nextEl: '.dates-home__nav .dates-home__arrow_next',
            prevEl: '.dates-home__nav .dates-home__arrow_prev',
        },
...
if (document.querySelector('.another__slider')) {
    new Swiper('.another__slider', {
        /*
        effect: 'fade',
        fadeEffect: {
            crossFade: true
        ...
        */
        ...
        // Arrows
        navigation: {
            nextEl: '.another__nav .slider-arrow_next',
            prevEl: '.another__nav .slider-arrow_prev',
        },
...
if (document.querySelector('.slider-inner__body')) {
    new Swiper('.slider-inner__body', {
        observer: true,
        observeParents: true,
        slidesPerView: 1,
        spaceBetween: 20,
        ...
        // Arrows
        navigation: {
            nextEl: '.slider-inner__nav .slider-arrow_next',
            prevEl: '.slider-inner__nav .slider-arrow_prev',
        },
        pagination: {
